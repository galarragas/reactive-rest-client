...
class SimpleSprayClientTest extends FlatSpec with Matchers {
  ...
  ignore should "limit parallel requests -- in this way can't work because of the way WireMock handles the requests and count calls" in
    withStubbedApi() { client : SimpleSprayClient =>

      val responseDelay = defaultFrequency.interval / 2
      givenThat {
        get( urlMatching( fakeServiceRegex ) ) willReturn {
          aResponse withStatus(SC_OK)  withHeader(CONTENT_TYPE, "application/json") withBody( """{ "message": "hello" }""" ) withFixedDelay(responseDelay.toMillis.toInt)
        }
      ...
      for { id <- 0 to (defaultFrequency.amount * 2) } yield client.callFakeService(id)

      Thread.sleep( (responseDelay / 3).toMillis )

      verify(defaultParallelRequests, getRequestedFor( urlMatching( fakeServiceRegex ) ) )

      Thread.sleep(responseDelay.toMillis + 1000)

      verify(defaultFrequency.amount - defaultParallelRequests, getRequestedFor( urlMatching( fakeServiceRegex ) ) )
